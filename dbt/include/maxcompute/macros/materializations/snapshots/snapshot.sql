...
{% macro build_snapshot_staging_table(strategy, sql, target_relation) %}
    ...
    {% call statement('build_snapshot_staging_relation') %}
        {{ create_table_as_internal(True, temp_relation, select, True, tblproperties={"table.format.version": "2"}) }}
    {% endcall %}
...
{% materialization snapshot, adapter='maxcompute' %}
  ...
  {% if not target_relation_exists %}
      ...
      {% set final_sql = create_table_as_internal(False, target_relation, build_sql, True, tblproperties={"table.format.version": "2"}) %}
